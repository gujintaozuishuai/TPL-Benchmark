...
def find_project_root(directory):
    for root, dirs, files in os.walk(directory):
        if 'build.gradle' in files and 'settings.gradle' in files:
            return root
    return None
...
# 更新代码报错，不知道怎么兼容那一种特殊情况，一定要考虑到兼容之前的特殊情况，而不是直接处理当前的特殊情况，这是难点，也是麻烦的地方
...
def extract_dependencies(file_path, root_properties, root_ext, root_toml):
    ...
    submodule_pattern = re.compile(
        ...
        r'(annotationProcessor|compile|implementation|api|compileOnly|runtimeOnly)\s+project\s*\(\s*["\'](?::[\w\-.]+)*:([\w\-.]+)["\']\s*\)|'
        r'(annotationProcessor|compile|implementation|api|compileOnly|runtimeOnly)\s+project\s*\(\s*path:\s*["\'](?::[\w\-.]+)*:([\w\-.]+)["\']\s*\)'
    )
    ...
    return resolved_dependencies, submodule_dependencies, content
...
def parse_libraries(libraries, versions, bundles):
    ...
    for bundle, libs in bundles.items():
        ...
        for lib in libs:
            ...
            lib = lib.replace('_', '.')
            if lib in result:
                bundle_libraries.append(result[lib])
        result[bundle_key] = bundle_libraries
    ...
    for root, dirs, files in os.walk(directory_path):
        for file in files:
            if file == 'build.gradle':
                file_path = os.path.join(root, file)
                module_name = os.path.basename(root)
                dependencies, submodules, content = extract_dependencies(file_path, root_properties, root_ext, root_toml)
...
def load_and_filter_csv(file_path):
    ...
    with open(file_path, 'r', encoding='utf-8') as file:
        ...
        for row in reader:
            if int(row['build.gradle Count']) == 2 and int(row['build.gradle.kts Count']) == 0:
...
def main():
    csv_file_path = r'update_analysis_releases_build_files_details.csv'
    base_directory = r'E:/unzip_all_releases_apk_src'
...
def main():
    initial_directory = r'H:\src_with_apk\zfdang_zSMTH-Android'
